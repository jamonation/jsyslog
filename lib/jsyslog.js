...
var server_sock = net.createServer(read_sock)
  // do the needful, bind to the unix socket
  server_sock.listen(config.socketPath)

var server_dgram = dgram.createSocket('udp4')
  // do the needful, bind the udp port
  server_dgram.bind(config.port, config.iface)

var logFile = fs.createWriteStream(config.syslogFile, {
                                   flags: 'a', mode: '0640',
                                   encoding: 'utf8'})

logFile
  .on('open', function() {
    var message = helpers.create_message('opened ' + config.syslogFile +
                                         ' for logging')
  ...
  .on('error', function(err) {
    ...
    if (err.errno === 'EADDRINUSE' && config.claimSocket === true) {
      ...
      fs.unlink(config.socketPath, function(err) {
        if (err) {
          logger.error(helpers.create_message('could not unlink ' +
                                              config.socketPath))
        } else {
          logger.info(helpers.create_message('unlinked old socket at ' +
                                             config.socketPath))
          server_sock.listen(config.socketPath)
        }
      })
...
var shutdown = function() {
  ...
  if (queue.items.length === 0) {  
    server_sock.close( function(err) {
      if (err) {
        logger.error(helpers.create_message('Could not close socket!! ' +
                                            config.socketPath))
        process.exit(1)
      }
    })
    server_dgram.close()
    var message = helpers.create_message('Cleaned up socket ' +
                                         config.socketPath + ', exiting\n')
